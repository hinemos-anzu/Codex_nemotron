...
## Prompt6 copy-to-root notebook creation

Execution datetime: 2026-06-08 UTC in the local `/workspace/Codex_nemotron` environment.

```bash
python - <<'PY'
# Generate experiments/b3_gate15_x17/b3_gate15_x17_kaggle_prompt6_copy_to_root.ipynb
# for Kaggle Prompt6 copy-to-root only; no submit.
PY
python - <<'PY'
import json
from pathlib import Path
p=Path('experiments/b3_gate15_x17/b3_gate15_x17_kaggle_prompt6_copy_to_root.ipynb')
nb=json.loads(p.read_text())
for i,c in enumerate(nb['cells'],1):
    if c['cell_type']=='code':
        compile(''.join(c['source']), f'cell{i}', 'exec')
text=p.read_text()
for term in ['Kaggle Submit: not performed','safe_for_manual_kaggle_submit','copy2','diagnostic_summary.json','copy_to_root_check.json','copy_to_root_report.md']:
    assert term in text, term
print('prompt6 notebook validation passed', len(nb['cells']))
PY
python -m py_compile experiments/b3_gate15_x17/b3_gate15_x17.py
rm -rf experiments/b3_gate15_x17/__pycache__
git diff --name-only -- b3-nemotron-svd-26042701.ipynb
...
Prompt6 notebook creation explicitly did **not** run:

```bash
# No execution of the generated Prompt6 notebook in local Codex
# No copy to /kaggle/working/submission.zip in local Codex
# No adapter generation
# No adapter_config.json generation
# No submission.zip generation or regeneration
# No SFT/training
